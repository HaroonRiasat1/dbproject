...
<h1>Select option to check your progress</h1>
	  <form method="post" action="" name="myForm" onsubmit="return validateForm()">
			  <div class="form-row">
				  <div class ="col-lg-7">
				
				    
  <input class ="col-lg-0" type="radio" name="sel" value="Daily">Daily <br>  
  <input class ="col-lg-0" type="radio" name="sel" value="Weekly"> Weekly <br>  
  <input  class ="col-lg-0"type="radio" name="sel" value="Monthly">Monthly <br>  
				    ...
				    <div class="form-row">
				  <div class ="col-lg-7">
					  <input type="submit" name="submit12" class="btn1 mt-3 mb-5" value="Enter"/>
...
$q="select TIMESTAMP,EXERCISE_ID,HEIGHT,WEIGHT,EXERCISE_PROG,NUTRITION_PROG from PROJ_BODYDATA where MEM_ID='$id' ";
$query_id = oci_parse($con, $q); 		
		$r = oci_execute($query_id);
...
$checker=0;
	  ...
	  while($row = oci_fetch_array($query_id, OCI_BOTH+OCI_RETURN_NULLS)) 
		  ...
		  echo "<td>" . $row['EXERCISE_PROG'] . "</td>";
		  $p=$row['EXERCISE_PROG'] ;
		  $n=$row['NUTRITION_PROG'] ;
		  $height=$row['HEIGHT'];
		  $weight=$row['WEIGHT'];

		  $BMI =$weight/sqrt($height);

		  echo "<td>" . $row['NUTRITION_PROG'] . "</td>";
		   echo "<td>" . $BMI. "</td>";
		  ...
		  echo "<label for='course'>Exerxise completion:</label> <progress id='course' value='$p' max='100'></progress>";
		  echo "<label for='course'>Nutrition completion:</label> <progress id='course' value='$n' max='100'></progress>";
