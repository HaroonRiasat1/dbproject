...
<h1>Please enter details of Exercise Plans </h1>
	  <form method="post" action="" name="myForm" onsubmit="return validateForm()">
			  <div class="form-row">
				  ...
				  <input type="text" placeholder="Name" name="exercisename" class="form-control my-3 p-4">
				  ...
				  <input type="number" placeholder="Daily Time" name="time" class="form-control my-3 p-4">
				  ...
				  <input type="number" placeholder="Daily Ammount" name="ammount" class="form-control my-3 p-4">
...
$q="insert into proj_exercise values(ex.nextval,'$time','$name','$Description','$Equipment','$DAY')";
$query_id = oci_parse($con, $q); 		
$r = oci_execute($query_id);


$q="select EXERCISE_ID from proj_exercise order by EXERCISE_ID";
	$query_id = oci_parse($con, $q); 		
$r = oci_execute($query_id);
 while($row = oci_fetch_array($query_id, OCI_BOTH+OCI_RETURN_NULLS)) 
      	  {
...
$q="insert into PROJ_DEMANDS VALUES('$Pid',pd.nextval,'$eid',$ammount,'')";
$query_id = oci_parse($con, $q); 		
$r = oci_execute($query_id);
